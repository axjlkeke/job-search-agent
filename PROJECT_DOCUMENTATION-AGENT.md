...
- 主色采用克制的蓝紫渐变、白色和浅灰，深色仅用于文字与关键决策；避免多套互相冲突的高饱和颜色和廉价发光效果。
...
| 素材 | 使用位置 | 是否允许复用 |
| --- | --- | --- |
| `01-report-hero-background.png` | 求职报告长幅标题区 | 仅报告标题区及同层级宣传画面 |
| `02-school-resource-folders.png` | 学校情况的求职资源卡 | 可复用到院校资源详情 |
| `03-advice-target-path.png` | 建议章综合建议卡 | 可复用到路径总结，不用于普通图标 |
| `04-profile-dossier.png` | 个人资料首屏 | 可复用到建档空状态 |
| `05-chat-guidance.png` | 顾问连接/空状态 | 可复用到对话空状态 |
| `06-roadmap-timeline-background.png` | 时间线版路线背景 | 仅路线主视图 |
| `07-recruitment-network-background.png` | 招聘线版路线背景 | 仅路线主视图 |
| `08-school-employer-network.png` | 学校情况校招资源概览 | 仅学校与雇主网络说明 |
| `09-school-career-pathways.png` | 学校情况三条就业路径 | 仅专业方向与岗位路径说明 |
| `10-school-recruitment-gateway.png` | 学校可接触的重点雇主 | 仅历史校招入口与雇主覆盖说明 |
| `11-school-evidence-kit.png` | 专业平台与校内资源 | 可复用到院校资源详情 |
| `12-school-career-impact.png` | 院校优势综合判断 | 仅学校情况综合判断和同层级总结 |

素材不包含文字和品牌 Logo，文案、数字和状态始终由代码渲染。需要随数据变化、需要交互或需要响应式重排的内容不得烘焙进图片。
